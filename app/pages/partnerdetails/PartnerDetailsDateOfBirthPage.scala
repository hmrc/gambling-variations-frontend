...
case class PartnerDetailsDateOfBirthPage(businessNumberOrIndex: BusinessNumberOrIndex) extends QuestionPage[LocalDate] {
  ...
  override def toString: String = "partnerDetailsDateOfBirth"
}

object PartnerDetailsDateOfBirthPage {
  def apply(index: Int) = new PartnerDetailsDateOfBirthPage(index)
  def apply(partnerDetailsBusinessNumber: String) = new PartnerDetailsDateOfBirthPage(partnerDetailsBusinessNumber)
}
