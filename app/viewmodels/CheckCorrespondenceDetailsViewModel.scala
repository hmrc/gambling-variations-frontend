      ...
      actions = if (correspondenceName.isEmpty) {
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceNameController.onPageLoad(NormalMode).url,
                content            = "site.change",
        ...
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceNameController.onPageLoad(CheckMode).url,
                content            = "site.change",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.correspondenceName.hidden"))
              )
            )
      ...
      SummaryListRow(
        ...
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceAdditionalNameYesNoController.onPageLoad(CheckMode).url,
                content            = "site.change",
      ...
      actions = if (additionalCorrespondenceName.isEmpty) {
        ...
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceAdditionalNameController.onPageLoad(CheckMode).url,
                content            = "site.change",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.additionalCorrespondenceName.hidden"))
              ),
              ActionItem(
                href               = controllers.routes.RemoveAdditionalCorrespondenceNameYesNoController.onPageLoad(CheckMode).url,
                content            = "site.remove",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.additionalCorrespondenceName.hidden"))
              )
            )
      ...
      actions = if (correspondenceName.isEmpty) {
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceChangeAddrScreenerController.onPageLoad(CheckMode).url,
                content            = "site.change",
        ...
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceUKAddressController.onPageLoad(CheckMode).url,
                content            = "site.change",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.correspondenceAddress.hidden"))
              )
            )
      ...
      SummaryListRow(
        ...
        actions = Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceAddrInfoScreenerController.onPageLoad(CheckMode).url,
                content            = "site.change",
      ...
      actions = if (correspondenceAdditionalInformation.isEmpty) {
        ...
        Some(
          Actions(
            items = Seq(
              ActionItem(
                href               = controllers.routes.CorrespondenceAdditionalInfoController.onPageLoad(CheckMode).url,
                content            = "site.change",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.additionalCorrespondenceInformation.hidden"))
              ),
              ActionItem(
                href               = controllers.routes.RemoveCorrAddressAddInfoController.onPageLoad(CheckMode).url,
                content            = "site.remove",
                visuallyHiddenText = Some(messages("checkCorrespondenceDetails.label.additionalCorrespondenceInformation.hidden"))
              )
            )
