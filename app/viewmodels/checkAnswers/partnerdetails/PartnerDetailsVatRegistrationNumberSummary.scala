...
object PartnerDetailsVatRegistrationNumberSummary {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PartnerDetailsVrnPage(index)).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ...
          ActionItemViewModel("site.change", controllers.partnerdetails.routes.PartnerDetailsVatRegistrationNumberController.onPageLoad(???, ???).url)
