...
object PartnerDetailsTradingNameSummary {
  ...
  def row(answers: UserAnswers, index: Int)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PartnerDetailsTradingNamePage(index)).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          // TODO important
          ActionItemViewModel("site.change", controllers.partnerdetails.routes.PartnerDetailsTradingNameController.onPageLoad(???, ???).url)
