...
object PartnerDetailsAddEmailAddressYesNoPageSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] = {
    val index: String = ???

    answers.get(PartnerDetailsAddEmailAddressYesNoPage(index)).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change",
                              controllers.partnerdetails.routes.PartnerDetailsAddEmailAddressYesNoPageController.onPageLoad(???).url
                             ) // TODO
