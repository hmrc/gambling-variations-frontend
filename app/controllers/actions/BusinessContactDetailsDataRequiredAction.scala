...
import models.{BusinessContactDetails, BusinessContactNumber, UserAnswers}
import pages.*
...
class BusinessContactDetailsDataRequiredActionImpl @Inject() (
  ...
  override protected def refine[A](request: OptionalDataRequest[A]): Future[Either[Result, DataRequest[A]]] = {
    request.userAnswers match {
      ...
      case Some(userAnswers) =>
        ...
        userAnswers.get(BusinessContactDetailsMgdRegNumPage) map { _ =>
  ...
  private def setBusinessContactDetails(contact: BusinessContactDetails, answers: UserAnswers): Try[UserAnswers] = {
    ...
    for {
      updatedAnswers <- answers.set(BusinessContactDetailsMgdRegNumPage, contact.mgdRegNumber)
