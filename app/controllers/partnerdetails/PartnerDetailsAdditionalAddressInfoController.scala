...
class PartnerDetailsAdditionalAddressInfoController @Inject() (
  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers
      .get(PartnerDetailsAdditionalAddressInfoPage(newIndex))
      .fold(form)(form.fill)
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsAdditionalAddressInfoPage(newIndex), value))
            updatedAnswers <- Future.fromTry(updatedAnswers.set(PartnerDetailsSubmittedPage(newIndex), true))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsAdditionalAddressInfoPage(newIndex), mode, updatedAnswers))
      )
  }
}
