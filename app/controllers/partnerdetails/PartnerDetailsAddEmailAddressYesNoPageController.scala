...
class PartnerDetailsAddEmailAddressYesNoPageController @Inject() (
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsAddEmailAddressYesNoPage(newIndex), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsAddEmailAddressYesNoPage(newIndex), mode, updatedAnswers))
      )
  }
}
