...
class PartnerDetailsSoleProprietorDobController @Inject() (
  ...
  view: PartnerDetailsSoleProprietorDobView
  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] =
    (authorise andThen getData andThen requireData) { implicit request =>
      ...
      if (!shouldShowPartnerSoleProprietorDob(request.userAnswers, newIndex)) {
        ...
        val preparedForm =
          request.userAnswers
            .get(PartnerDetailsDateOfBirthPage(newIndex))
            .fold(form)(form.fill)
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsDateOfBirthPage(newIndex), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsDateOfBirthPage(newIndex), mode, updatedAnswers))
      )
  }

  private def shouldShowPartnerSoleProprietorDob(userAnswers: UserAnswers, index: BusinessNumberOrIndex): Boolean =
