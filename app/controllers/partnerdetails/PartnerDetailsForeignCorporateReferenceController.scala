  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData) { implicit request =>
    ...
    (businessType, incorporatedInUK) match {
      case (Some(Corporatebody), Some(false)) =>
        val preparedForm = request.userAnswers.get(PartnerDetailsForeignCorporateReferencePage(newIndex)) match {
          case None        => form
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsForeignCorporateReferencePage(newIndex), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsForeignCorporateReferencePage(newIndex), mode, updatedAnswers))
      )
  }
}
