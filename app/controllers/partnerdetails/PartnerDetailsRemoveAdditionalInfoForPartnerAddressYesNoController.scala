...
class PartnerDetailsRemoveAdditionalInfoForPartnerAddressYesNoController @Inject() (
  ...
  view: PartnerDetailsRemoveAdditionalInfoForPartnerAddressYesNoView
          ...
          form
            ...
            .fold(
              formWithErrors =>
                Future.successful(
                  BadRequest(
                    view(formWithErrors, index, mode, additionalInformation)
                  )
                ),
              value =>
                for {
                  updatedAnswers <- Future.fromTry(
                                      request.userAnswers.set(
                                        PartnerDetailsRemoveAdditionalInfoForPartnerAddressYesNoPage(newIndex),
                                        value
                                      )
                                    )
