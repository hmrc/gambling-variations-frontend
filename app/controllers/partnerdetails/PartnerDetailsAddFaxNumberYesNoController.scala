...
class PartnerDetailsAddFaxNumberYesNoController @Inject() (
  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(PartnerDetailsAddFaxNumberYesNoPage(newIndex)) match {
      case None        => form
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsAddFaxNumberYesNoPage(newIndex), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsAddFaxNumberYesNoPage(newIndex), mode, updatedAnswers))
      )
  }
}
