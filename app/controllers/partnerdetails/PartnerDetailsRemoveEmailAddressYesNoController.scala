...
import pages.partnerdetails.{PartnerDetailsCorrespondenceEmailAddressPage, PartnerDetailsRemoveEmailAddressYesNoPage}
        ...
        value =>
          for {
            updatedAnswers <- if (value) {
                                Future.fromTry(request.userAnswers.remove(PartnerDetailsCorrespondenceEmailAddressPage(newIndex)))
                              } else {
                                Future.apply(request.userAnswers)
                              }
            updatedAnswers <- Future.fromTry(updatedAnswers.set(PartnerDetailsRemoveEmailAddressYesNoPage(newIndex), value))
