...
import forms.partnerdetails.PartnerDetailsAddNationalInsuranceNumberYesNoFormProvider
import models.Mode
...
import views.html.partnerdetails.PartnerDetailsAddNationalInsuranceNumberYesNoView
...
class PartnerDetailsAddNationalInsuranceNumberYesNoController @Inject() (
  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(PartnerDetailsAddNationalInsuranceNumberYesNoPage(newIndex)) match {
      case None        => form
  ...
  def onSubmit(index: String, mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    ...
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsAddNationalInsuranceNumberYesNoPage(newIndex), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsAddNationalInsuranceNumberYesNoPage(newIndex), mode, updatedAnswers))
      )
  }
}
