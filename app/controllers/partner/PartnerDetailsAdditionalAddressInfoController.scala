...
class PartnerDetailsAdditionalAddressInfoController @Inject() (
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers
      .get(PartnerDetailsAdditionalAddressInfoPage)
      .fold(form)(form.fill)
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (authorise andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PartnerDetailsAdditionalAddressInfoPage, value))
            updatedAnswers <- Future.fromTry(updatedAnswers.set(PartnerDetailsSubmittedPage, true))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PartnerDetailsAdditionalAddressInfoPage, mode, updatedAnswers))
      )
  }
}
