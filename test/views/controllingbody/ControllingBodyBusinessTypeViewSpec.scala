...
class ControllingBodyBusinessTypeViewSpec extends SpecBase {

  private val form = new ControllingBodyBusinessTypeFormProvider()()

  "ControllingBodyBusinessTypeView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        document.title() must include(
          messages(application)("controllingBodyBusinessType.title")
        )

        document.select("h1").select(".govuk-fieldset__heading").text() mustEqual
          messages(application)("controllingBodyBusinessType.heading")
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        document.body().text() must include(
          messages(application)("controllingBodyBusinessType.error.required")
        )
      }
    }
  }
