...
class RemoveLicenceNumberViewSpec extends SpecBase {

  trait Setup {
    private val app = applicationBuilder().build()

    private val view = app.injector.instanceOf[RemoveLicenceNumberView]
    ...
    val licenceNumber = "123456"

    private val html = view(form, NormalMode, licenceNumber)(request, messages)

    val doc: Document = Jsoup.parse(html.body)

  }

  "RemoveLicenceNumberView" - {

    "must render page correctly" in new Setup {

      doc.title must include(messages("removeLicenceNumber.title"))

      doc.select("span").select(".govuk-caption-l").text() must include(messages("changeRegistrationDetails.caption"))
      doc.select("legend.govuk-fieldset__legend").text()   must include(messages("removeLicenceNumber.heading", licenceNumber))
