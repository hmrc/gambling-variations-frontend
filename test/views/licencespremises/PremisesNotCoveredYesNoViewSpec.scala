...
class PremisesNotCoveredYesNoViewSpec extends SpecBase {

  private val form = new PremisesNotCoveredYesNoFormProvider()()

  "PremisesNotCoveredYesNoView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        document.title() must include(
          messages(application)("premisesNotCoveredYesNo.title")
        )

        document.select("h1").select(".govuk-fieldset__heading").text() mustEqual
          messages(application)("premisesNotCoveredYesNo.heading")
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        document.body().text() must include(
          messages(application)("premisesNotCoveredYesNo.error.required")
        )
      }
    }
  }
