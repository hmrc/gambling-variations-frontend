...
class LicenceDetailsLandlordLicenceYesNoFormProviderSpec extends SpecBase {

  private val form = new LicenceDetailsLandlordLicenceYesNoFormProvider()()

  "LicenceDetailsLandlordLicenceYesNoView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        document.title() must include(
          messages(application)("licenceDetailsLandlordLicenceYesNo.title")
        )

        document.select("h1").select(".govuk-fieldset__heading").text() mustEqual
          messages(application)("licenceDetailsLandlordLicenceYesNo.heading")
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        document.body().text() must include(
          messages(application)("licenceDetailsLandlordLicenceYesNo.error.required")
        )
      }
    }
  }
