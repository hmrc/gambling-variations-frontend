...
class PartnerDetailsAddCountryOfIncorporationViewSpec extends SpecBase {

  private val formProvider = new PartnerDetailsAddCountryOfIncorporationFormProvider()
  private val form = formProvider()

  "PartnerDetailsAddCountryOfIncorporationView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        document.title() must include(
          messages(application)("partnerDetailsAddCountryOfIncorporation.title")
        )
        ...
        document.body().text() must include(
          messages(application)("partnerDetailsAddCountryOfIncorporation.heading")
        )
        ...
        val input = document.select("input[name=value]")
        input.size() mustEqual 1
        input.hasClass("govuk-input--width-100") mustBe true
        input.attr("autocomplete") mustEqual "country-name"
      }
    }
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        document.body().text() must include(
          messages(application)("partnerDetailsAddCountryOfIncorporation.error.required")
        )
      }
    }
  }
