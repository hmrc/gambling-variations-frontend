...
class PartnerDateOfJoiningViewSpec extends SpecBase {
  ...
  "PartnerDateOfJoiningView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        val formProvider =
          application.injector
            .instanceOf[
              PartnerDateOfJoiningFormProvider
            ]
        ...
        document.title() must include(
          msgs("partnerDateOfJoining.title")
        )

        document
          .select("h1")
          .text() mustEqual
          msgs("partnerDateOfJoining.heading")
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        document.body().text() must include(
          msgs(
            "partnerDateOfJoining.error.required.all"
          )
        )
    ...
    "render the latest permitted date in the hint" in {
