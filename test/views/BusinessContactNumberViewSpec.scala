...
class BusinessContactNumberViewSpec extends SpecBase {
  ...
  trait Setup {
    ...
    val view = app.injector.instanceOf[BusinessContactNumberView]
  ...
  "BusinessContactNumberView" - {

    "must render page correctly" in new Setup {
      doc.title must include(messages("businessContactNumber.title"))

      doc.select("h1").text must include(messages("businessContactNumber.heading"))

      doc.select("p").select(".govuk-body").text must include(messages("businessContactNumber.p1"))
