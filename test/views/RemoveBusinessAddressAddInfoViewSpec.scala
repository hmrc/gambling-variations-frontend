...
class RemoveBusinessAddressAddInfoViewSpec extends SpecBase {

  "RemoveBusinessAddressAddInfoView" - {
    ...
    "must render page correctly" in new Setup {
      ...
      doc.title must include(messages("removeBusinessAddressAddInfo.title"))

      doc.select("legend").text must include(
        messages("removeBusinessAddressAddInfo.heading")
      )

      doc.select("p").text must include(
        messages("removeBusinessAddressAddInfo.paragraph", addressAddInfo)
      )
  ...
  trait Setup {
    ...
    val view = app.injector.instanceOf[RemoveBusinessAddressAddInfoView]
