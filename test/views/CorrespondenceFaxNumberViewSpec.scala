...
class CorrespondenceFaxNumberViewSpec extends SpecBase {

  trait Setup {
    private val app = applicationBuilder().build()

    private val view = app.injector.instanceOf[CorrespondenceFaxNumberView]
  ...
  "CorrespondenceFaxNumberView" - {

    "must render page correctly" in new Setup {

      doc.title must include(messages("correspondenceFaxNumber.title"))

      doc.select("h1").select(".govuk-label--l").text()    must include(messages("correspondenceFaxNumber.heading"))
