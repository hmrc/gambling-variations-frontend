...
class PartnerDetailsAddNationalInsuranceNumberViewSpec extends SpecBase {
  ...
  "PartnerDetailsAddNationalInsuranceNumberView" - {
    ...
    "must render page correctly" in new Setup {
      ...
      val html: HtmlFormat.Appendable = view(form, newPartnersIndex, NormalMode)(request, messages)
    ...
    "must not render error summary when the last char is missing" in new Setup {
      ...
      val html: HtmlFormat.Appendable = view(boundForm, newPartnersIndex, NormalMode)(request, messages)
      val doc: Document = Jsoup.parse(html.body)
