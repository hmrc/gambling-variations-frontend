...
class PartnerDetailsIsBusinessIncorporatedUkViewSpec extends SpecBase {

  private val newPartnersIndex = 0.toString
  private val form = new PartnerDetailsIsBusinessIncorporatedUkFormProvider()()

  "PartnerDetailsIsBusinessIncorporatedUkView" - {
    ...
    "render the page correctly" in {
      ...
      running(application) {
        ...
        document.title() must include(
          messages(application)("partnerDetailsIsBusinessIncorporatedUk.title")
        )

        document.select("h1").select(".govuk-fieldset__heading").text() mustEqual
          messages(application)("partnerDetailsIsBusinessIncorporatedUk.heading")
    ...
    "render an error summary when there are form errors" in {
      ...
      running(application) {
        ...
        val html = view(boundForm, newPartnersIndex, NormalMode)(FakeRequest(), messages(application))
        ...
        document.body().text() must include(
          messages(application)("partnerDetailsIsBusinessIncorporatedUk.error.required")
        )
      }
    }
  }
