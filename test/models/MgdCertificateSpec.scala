...
class MgdCertificateSpec extends AnyWordSpec with Matchers {
  ...
  "MgdCertificate JSON format" should {
    ...
    "PartnerMember JSON format" should {
      ...
      "serialize and deserialize all fields" in {
        ...
        val parsed = json.as[PartnerMember]

        parsed mustBe member
        (json \ "solePropMiddleName").as[String] mustBe "M"
      }
    }

    "ReturnPeriodEndDate JSON format" should {

      "serialize and deserialize LocalDate correctly" in {
        val date = ReturnPeriodEndDate(LocalDate.of(2024, 12, 31))
        val json = Json.toJson(date)
        val parsed = json.as[ReturnPeriodEndDate]

        parsed mustBe date
        (json \ "returnPeriodEndDate").as[String] mustBe "2024-12-31"
      }
    }
