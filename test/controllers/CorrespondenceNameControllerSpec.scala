...
class CorrespondenceNameControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new CorrespondenceNameFormProvider()
  ...
  lazy val nameRoute =
    routes.CorrespondenceNameController.onPageLoad(NormalMode).url

  "CorrespondenceName Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[CorrespondenceNameView]
    ...
    "must populate the view on a GET when the question has previously been answered" in {
      ...
      val data = Json.obj(
        ...
        CorrespondenceNamePage.toString -> "validName"
      )
