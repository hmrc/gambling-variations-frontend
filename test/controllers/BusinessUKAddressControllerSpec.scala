...
class BusinessUKAddressControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute: Call = Call("GET", "/foo")

  val formProvider = new BusinessUKAddressFormProvider()
  ...
  lazy val addressRoute: String =
    routes.BusinessUKAddressController.onPageLoad().url

  "BusinessUKAddress Controller" - {
    ...
    "must populate the view on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        noAnswers.set(BusinessAddressUkPage, validAddress).success.value
