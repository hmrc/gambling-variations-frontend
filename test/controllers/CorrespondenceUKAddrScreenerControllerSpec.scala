...
class CorrespondenceUKAddrScreenerControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CorrespondenceUKAddrScreener Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, correspondenceUKAddrScreenerRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[CorrespondenceUKAddrScreenerView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CorrespondenceUKAddrScreenerPage, true).success.value
