...
import pages.correspondencedetails.{CorrespondenceDetailsChangesPage, CorrespondenceEmailPage}
  ...
  "CorrespondenceEmailAddress Controller" - {
    ...
    "must update data correctly when submitted in" in {
      ...
      running(application) {
        ...
        savedAnswersCaptor.getValue.get(CorrespondenceEmailPage).value mustEqual "validEmail@example.com"
      }
    }
