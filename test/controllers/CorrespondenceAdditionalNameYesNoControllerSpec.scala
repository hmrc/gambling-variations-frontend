...
class CorrespondenceAdditionalNameYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CorrespondenceAdditionalNameYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, correspondenceAdditionalNameYesNoRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[CorrespondenceAdditionalNameYesNoView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, correspondenceAdditionalNameYesNoRoute)
