...
class OtherTradeClassControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new OtherTradeClassFormProvider()
  val form = formProvider()

  val noAnswers =
    UserAnswers(
      userAnswersId,
      Json.obj("otherTradeClassSection" -> Json.obj("mgdRegNum" -> userAnswersId))
    )
  ...
  "OtherTradeClass Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[OtherTradeClassView]
    ...
    "must populate the view on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual
          view(form.fill("valid trade class"), NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, otherTradeClassRoute)
            .withFormUrlEncodedBody(("otherTradeClass", ""))

        val boundForm = form.bind(Map("otherTradeClass" -> ""))
