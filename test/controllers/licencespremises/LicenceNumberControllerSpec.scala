...
class LicenceNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LicenceNumberController Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, licenceAndPremisesRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[LicenceNumberView]
    ...
    "must populate the view on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual
          view(form.fill("123-456789-A-123456-789"), NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, licenceAndPremisesRoute)
            .withFormUrlEncodedBody(("gamblingLicenceNo", "123-456789-A-123456-789"))
    ...
    "must update data correctly when submitted in" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, licenceAndPremisesRoute)
            .withFormUrlEncodedBody(("gamblingLicenceNo", "123-456789-A-123456-000"))
    ...
    "must flag LicencesPremisesDetailsChangesPage when data changed in" in {
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, licenceAndPremisesRoute)
            .withFormUrlEncodedBody(("gamblingLicenceNo", ""))

        val boundForm = form.bind(Map("gamblingLicenceNo" -> ""))
