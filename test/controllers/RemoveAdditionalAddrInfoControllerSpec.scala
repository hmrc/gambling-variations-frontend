...
class RemoveAdditionalAddrInfoControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removeAdditionalAddrInfoRoute = routes.RemoveAdditionalAddrInfoController.onPageLoad(NormalMode).url

  "RemoveAdditionalAddrInfo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, removeAdditionalAddrInfoRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[RemoveAdditionalAddrInfoView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, removeAdditionalAddrInfoRoute)
