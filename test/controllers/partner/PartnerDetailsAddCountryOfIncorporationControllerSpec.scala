...
class PartnerDetailsAddCountryOfIncorporationControllerSpec extends SpecBase with MockitoSugar with PartnerDetailsHelper {
  ...
  lazy val countryOfIncorporationRoute: String =
  ...
  "PartnerDetailsAddCountryOfIncorporation Controller" - {
    ...
    "onPageLoad" - {
      ...
      "must populate the view correctly on a GET when the question has previously been answered" in {
        ...
        val userAnswers = validUserAnswers
          .set(PartnerDetailsCountryOfIncorporationPage(index), validCountry)
          .success
          .value
        ...
        running(application) {
          ...
          status(result) mustBe OK
          contentAsString(result) mustBe view(form.fill(validCountry), NormalMode)(request, messages(application)).toString
        }
      }
    ...
    "onSubmit" - {
      ...
      "must update UserAnswers and redirect to the next page when valid data is submitted" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, PartnerDetailsAddCountryOfIncorporationController.onSubmit().url)
