...
class controllingBodyBusinessTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "controllingBodyBusinessType Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(controllingBodyBusinessTypePage, controllingBodyBusinessType.values.head).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, controllingBodyBusinessTypeRoute)
            .withFormUrlEncodedBody(("value", controllingBodyBusinessType.values.head.toString))
