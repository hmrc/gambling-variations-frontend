...
import pages.correspondencedetails.AddCorrespondenceFaxNumberPage
...
class FaxNumberForCorrespondenceYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  "FaxNumberForCorrespondenceYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, faxNumberForCorrespondenceYesNoRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[FaxNumberForCorrespondenceYesNoView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(AddCorrespondenceFaxNumberPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, faxNumberForCorrespondenceYesNoRoute)
