...
import pages.{BusinessContactNumberPage, ContactDetailsChangesPage, TradingDetailsChangesPage}
  ...
  "BusinessContactNumber Controller" - {
    ...
    "must update data correctly when submitted in" in {
      ...
      running(application) {
        ...
        savedAnswersCaptor.getValue.get(BusinessContactNumberPage).value mustEqual ContactNumber(
          Some("01632 960 001"),
          Some("07111 111 111")
        )
      }
