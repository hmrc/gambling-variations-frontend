...
class PremisesNotCoveredYesNoSummarySpec extends SpecBase {
  ...
  "PremisesNotCoveredYesNoSummary" - {

    "must return None when the question has not been answered" in {
      PremisesNotCoveredYesNoSummary.row(emptyUserAnswers) mustBe None
    }
    ...
    "must return the correct row when the answer is Yes" in {
      ...
      PremisesNotCoveredYesNoSummary.row(answers).value mustBe
        SummaryListRowViewModel(
          key   = "premisesNotCoveredYesNo.checkYourAnswersLabel",
          value = ValueViewModel("site.yes"),
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              controllers.licencespremises.routes.PremisesNotCoveredYesNoController.onPageLoad().url
            ).withVisuallyHiddenText(
              messages("premisesNotCoveredYesNo.change.hidden")
            )
          )
        )
    }
    ...
    "must return the correct row when the answer is No" in {
      val answers =
        emptyUserAnswers
          .set(PremisesNotCoveredYesNoPage, false)
          .success
          .value
